...
function usage() {
  console.error("Usage: strap one-shot run <task> --provider provider.json [--tools none|all|fs|process|web|agent|scripts|jsmcp] [--max-turns 8] [--dry-run] < context.json");
  process.exit(2);
}
...
writeJson(await runOneShot({ input, task, provider, toolsName, maxTurns, finalize, dryRun }));
