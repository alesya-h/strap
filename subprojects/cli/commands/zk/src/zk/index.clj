...
(defn reindex-notes [& [{:keys [scope db]}]]
  (let [db-path (or (not-empty db) (default-db-path))]
    ...
    (let [notes (visible-notes (or scope "all"))]
      (doseq [note notes]
        (call-index ["create" "--id" (get-in note [:meta :id]) "--title" (get-in note [:meta :title]) "--body" (:body note)
                     ...
                     "--author" (or (get-in note [:meta :author]) "agent") "--db" db-path]))
...
(defn index-search [command args]
  (let [{:keys [opts pos]} (parse-args args) query (or (:query opts) (str/join " " pos)) limit (or (:limit opts) "10")]
...
(defn last-text-event [state]
  (let [children (get-in state [:root :children] []) assistant (filter #(and (= "event" (:type %)) (= "assistant" (:from %)) (not-empty (:text %))) children)]
    (or (last assistant) (last (filter #(and (= "event" (:type %)) (not-empty (:text %))) children)))))

(defn remember-state [args]
  (let [{:keys [opts]} (parse-args args) raw (if-let [file (:file opts)] (slurp file) (read-stdin-if-any)) state (parse-json raw) event (last-text-event state)]
    ...
    (let [title (or (not-empty (:title opts)) (subs (first (str/split-lines (:text event))) 0 (min 80 (count (first (str/split-lines (:text event)))))))
          id (note-id) file (str (fs/path (zettel-dir "user") (str (slug title) "-" (subs id 3 11) ".md"))) timestamp (now)
          meta {:id id :title title :tags (parse-list (or (:tags opts) "session,summary")) :aliases [] :author (or (:author opts) "agent") :scope "user" :created_at timestamp :updated_at timestamp}]
...
(defn invoke-self [args] (run-strap-out (concat ["zk"] args)))
(defn maybe [condition values] (if condition values []))

(defn tool [_args]
  (let [req (parse-json (read-stdin-if-any)) action (or (:action req) "search_hybrid")]
    (print
      (case action
        "create" (invoke-self (concat ["create" "--scope" (or (:scope req) "user") "--title" (:title req) "--body" (or (:body req) "") "--tags" (json/generate-string (or (:tags req) [])) "--aliases" (json/generate-string (or (:aliases req) [])) "--author" (or (:author req) "agent")]))
        "update" (invoke-self (concat ["update" (:id req)] (maybe (:title req) ["--title" (:title req)]) (maybe (:body req) ["--body" (:body req)]) (maybe (:tags req) ["--tags" (json/generate-string (:tags req))]) (maybe (:aliases req) ["--aliases" (json/generate-string (:aliases req))])))
        "delete" (invoke-self ["delete" (:id req)])
        "list" (invoke-self (concat ["list" "--scope" (or (:scope req) "all") "--limit" (str (or (:limit req) 50))] (maybe (:tag req) ["--tag" (:tag req)])))
        ...
        "search_hybrid" (invoke-self (concat ["search-hybrid" (:query req) "--scope" (or (:scope req) "all") "--limit" (str (or (:limit req) 10))] (maybe (:db req) ["--db" (:db req)])))
        "reindex" (invoke-self (concat ["reindex" "--scope" (or (:scope req) "all")] (maybe (:db req) ["--db" (:db req)])))
