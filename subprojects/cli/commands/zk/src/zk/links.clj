...
(defn resolve-link-target [target visible]
  (let [lower (str/lower-case target) slugged (slug target)
        ...
        alias-match (filter (fn [note] (some #(or (= % target) (= (str/lower-case (str %)) lower) (= (slug %) slugged))
                                             (parse-list (get-in note [:meta :aliases])))) visible)]
    ...
    (fn [link]
      (let [candidates (remove #(= (get-in % [:meta :id]) (get-in note [:meta :id])) (resolve-link-target (:target link) visible))]
        (case (count candidates)
          ...
          (assoc link :status "ambiguous" :candidates (mapv note-ref candidates)
...
(defn decorate-note [note visible & [{:keys [include-body include-paths]}]]
  (let [links (resolve-outgoing-links note visible) backlinks (backlink-report note visible)
        ...
        result (merge (note-ref note)
                      {:tags (parse-list (get-in note [:meta :tags])) :aliases (parse-list (get-in note [:meta :aliases]))
