...
def read-input [file: string] { if $file == "-" { ^cat | from json } else { open $file } }
...
def main [--model: string = "current", --tools: string = "all", --max-turns: int = 8, --no-finalize, --max-tool-calls: int = 64, --file: string = "-"] {
  read-input $file | run-loop $model $tools $max_turns (not $no_finalize) $max_tool_calls | write-json
}
