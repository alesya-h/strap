...
export async function callMcpTool({ command, args = [], tool, input = {}, timeoutMs = 120000 }) {
  ...
  try {
    await client.start();
    await client.request("initialize", { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "strap", version: "0.1.0" } });
...
class StdioMcpClient {
  ...
  async start() {
    ...
    this.child.on("exit", (code, signal) => {
      if (this.pending.size) this.rejectAll(new Error(`MCP process exited ${code ?? signal}: ${this.stderr}`));
    });
  }
