...
export def apply [name: string, --actor: string = "assistant"] {
  $in | run apply $name --actor $actor
}

export def roots [] { with-env (env-map) { run roots } }

export def import-opencode [source: string = ""] {
  with-env (env-map) {
    if $source == "" { run import-opencode } else { run import-opencode $source }
  }
}
