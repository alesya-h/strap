...
fn chat-messages {|statev|
  var messages = [[&role=system &content=(actor-frame $statev)]]
  each {|event|
    set messages = (conj $messages [&role=(provider-role $statev $event) &content=(event-text $event)])
  } (history $statev)
  put $messages
}
