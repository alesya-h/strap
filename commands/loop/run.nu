...
def complete-once [model: string, tools: string] { $in | to json | run-external (strap-bin) llm complete "--model" $model "--tools" $tools | from json }
def process-tools-once [tools: string] { $in | to json | run-external (strap-bin) run-calls "--tools" $tools | from json }
def last-event [] { let h = ($in.history? | default []); if (($h | length) == 0) { null } else { $h | last } }
def last-calls [] { let event = ($in | last-event); if ($event == null) { [] } else { $event.calls? | default [] } }
...
def add-budget-exhausted [turns: int] {
  $in | upsert actors.strap { kind: "runtime", self: { public: "Local Strap harness." } } | update history { append { from: "strap", kind: "tool_budget_exhausted", text: $"Tool budget exhausted after ($turns) turn\(s\). Answer now using the gathered context. Do not request more tools." } }
}

def add-tool-budget-exhausted [calls: int] {
  $in | upsert actors.strap { kind: "runtime", self: { public: "Local Strap harness." } } | update history { append { from: "strap", kind: "tool_budget_exhausted", text: $"Tool call budget exhausted after ($calls) requested call\(s\). Answer now using the gathered context. Do not request more tools." } }
}

def run-loop [model: string, tools: string, max_turns: int, finalize: bool, max_tool_calls: int] {
  mut state = $in; mut final = false; mut tool_calls = 0; mut turn = 0
