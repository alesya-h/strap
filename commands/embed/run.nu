...
def run-path [] { root | path join commands embed run }

def env-map [] {
  {
    STRAP_ROOT: (root)
    STRAP_CMD_DIR: (root | path join commands embed)
...
export def --wrapped main [...args: string] {
  ...
  let input = $in
  with-env (env-map) { $input | to json | run-external (run-path) ...$argv } | from json
}
