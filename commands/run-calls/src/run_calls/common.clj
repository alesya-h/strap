...
(defn root []
  (str (fs/absolutize (env "STRAP_ROOT" (System/getProperty "user.dir")))))

(defn global-root []
  (let [xdg (env "XDG_CONFIG_HOME" (str (System/getProperty "user.home") "/.config"))]
    (str (fs/absolutize (env "STRAP_GLOBAL" (str xdg "/strap"))))))

(defn config-root []
  (env "STRAP_CONFIG" (str (fs/path (root) "config" "strap"))))

(defn project-root []
  (env "STRAP_PROJECT" (str (fs/path (workspace) ".strap"))))

(defn work-root []
  (env "STRAP_WORK" (str (fs/path (workspace) ".strap-user"))))
