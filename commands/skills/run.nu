...
def roots [] {
  let configured = ($env.STRAP_SKILL_PATH? | default "" | split row ":" | where {|item| $item != "" })
  let session = if (($env.STRAP_SESSION? | default "") == "") { [] } else { [($env.STRAP_SESSION | path join overlay skills)] }
  $configured | append $session | append [
    (($env.STRAP_WORK? | default ((pwd) | path join .strap-user)) | path join skills)
    (($env.STRAP_PROJECT? | default ((pwd) | path join .strap)) | path join skills)
    (($env.STRAP_CONFIG? | default (($env.STRAP_ROOT? | default (pwd)) | path join config strap)) | path join skills)
    (($env.STRAP_ROOT? | default (pwd)) | path join skills)
  ]
}
